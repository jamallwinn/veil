...
/**
 * @title PoseidonT3
 * @notice Pure Solidity Poseidon hash function - XRPL EVM Compatible
 * @dev Based on https://github.com/yuriko627/poseidon-sol
 *      Referenced from https://github.com/iden3/circomlibjs/blob/v0.0.8/src/poseidon_slow.js
 *
 * This implementation:
 * - Uses PURE SOLIDITY (no inline assembly)
 * - Compatible with XRPL EVM (Evmos/Cosmos SDK based)
 * - Produces IDENTICAL outputs to circomlib's Poseidon(2) template
 * - BN254 curve (alt_bn128) field arithmetic
 * - 8 full rounds + 57 partial rounds
 *
 * Gas cost: ~21,000 gas per hash (higher than assembly version but compatible)
 */
library PoseidonT3 {
    // BN254 scalar field modulus
    uint256 constant F = 0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001;

    // Poseidon parameters from whitepaper https://eprint.iacr.org/2019/458.pdf
    uint256 constant N_ROUNDS_F = 8;   // Full rounds
    uint256 constant N_ROUNDS_P = 57;  // Partial rounds

    // MDS Matrix constants (3x3 matrix for T=3)
    ...
    /**
     * @notice Compute x^5 in the BN254 field
     * @param n The input value
     * @return n^5 mod F
     */
    function _pow5(uint256 n) private pure returns (uint256) {
     ...
     */
    function poseidon(uint256 left, uint256 right) internal pure returns (uint256) {
    ...
    /**
     * @notice Hash an array of two field elements using Poseidon
     * @param inputs Array of 2 input elements
     * @return The Poseidon hash
     */
    function hash(uint256[2] memory inputs) internal pure returns (uint256) {
        // Round constants (195 = 65 rounds * 3 state elements)
        ...
        for (uint256 r = 0; r < N_ROUNDS_F + N_ROUNDS_P; r++) {
            // Add round constants
            state[0] = addmod(res[0], C[r * t + 0], F);
            state[1] = addmod(res[1], C[r * t + 1], F);
            state[2] = addmod(res[2], C[r * t + 2], F);

            // S-box layer
            // Full rounds: apply S-box to all state elements
            // Partial rounds: apply S-box only to state[0]
            ...
            res[0] = addmod(
                addmod(mulmod(M00, state[0], F), mulmod(M01, state[1], F), F),
                mulmod(M02, state[2], F),
                F
            );
            res[1] = addmod(
                addmod(mulmod(M10, state[0], F), mulmod(M11, state[1], F), F),
                mulmod(M12, state[2], F),
                F
            );
            res[2] = addmod(
                addmod(mulmod(M20, state[0], F), mulmod(M21, state[1], F), F),
                mulmod(M22, state[2], F),
                F
            );
        }
